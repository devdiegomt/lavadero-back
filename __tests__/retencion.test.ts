...
/**
 * Las tareas de retención recorren **todos** los lavaderos: son de fondo, no de
 * una petición. En producción el cron las corre con el bypass de RLS explícito,
 * y acá se hace igual — usar un contexto de tenant probaría algo que no ocurre.
 */
const itCruzandoTenants = (nombre: string, fn: () => Promise<void>): void => {
  it(nombre, () => cruzandoTenants(fn));
};
...
beforeAll(async () => {
  ...
  const { rows: srv } = await db.queryAdmin<{ id: string }>(
...
afterEach(async () => {
  await db.queryAdmin(`DELETE FROM whatsapp_messages WHERE content LIKE $1`, [`${MARCA}%`]);
});
...
afterAll(async () => {
  ...
  await db.queryAdmin(`DELETE FROM customers WHERE tenant_id = $1 AND anonymized_at IS NOT NULL`, [
    tenantId,
  ]);
...
describe('purga de conversaciones', () => {
  ...
  async function existe(id: string): Promise<boolean> {
    const { rows } = await db.queryAdmin(`SELECT 1 FROM whatsapp_messages WHERE id = $1`, [id]);
    return rows.length === 1;
  }

  itCruzandoTenants('borra lo que pasó el plazo y conserva lo que no', async () => {
  ...
  itCruzandoTenants('con plazo 0 no borra nada: es la forma de desactivar la tarea', async () => {
...
describe('anonimización por inactividad', () => {
  itCruzandoTenants('borra los datos del inactivo y deja intacto al que sigue viniendo', async () => {
  ...
  itCruzandoTenants('con plazo 0 no anonimiza nada', async () => {
    expect(await anonimizarClientesInactivos(0)).toBe(0);
  });
});

describe('derecho de supresión', () => {
  itCruzandoTenants('anonimiza al titular sin perder sus turnos', async () => {
    const customerId = await crearCliente(LID_SUPRESION, 0);
    const { rows: veh } = await db.queryAdmin<{ id: string }>(
    ...
    const { rows: turnos } = await db.queryAdmin(
  ...
  itCruzandoTenants('no deja que un lavadero borre el cliente de otro', async () => {
    ...
    const { rows } = await db.queryAdmin<{ phone: string | null }>(
  ...
  itCruzandoTenants('sobre un cliente ya anonimizado no vuelve a actuar', async () => {
...
describe('pasivo de autorizaciones', () => {
  itCruzandoTenants('no cuenta a los anonimizados: ya no hay dato que regularizar', async () => {
    const { rows } = await db.queryAdmin<{ id: string }>(
